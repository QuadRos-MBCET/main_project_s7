...
# =====================================================================
    ...
    if st.session_state["current_tab"] == "🏠 Feed":
        ...
        conn = get_connection()
        cursor = conn.cursor()
        # Query photo posts (not videos) - include all statuses to let adults view restricted/rejected ads
        cursor.execute("""
            SELECT A.id, A.title, A.caption, A.file_path, U.username, A.status 
        ...
        filtered_posts = []
        for post in posts:
            pid, ptitle, pcap, pfile, puser, pstatus = post
            full_txt = f"{ptitle} {pcap or ''} {pfile}"
            
            # Identify if it is restricted (rejected, under review, or manually age restricted)
            is_flagged = (pstatus in ['rejected', 'under_review']) or is_age_restricted(full_txt)
            
            # If viewer is a child, completely filter out restricted/flagged ads
            if age_category == "Child" and is_flagged:
                continue
                
            filtered_posts.append((pid, ptitle, pcap, pfile, puser, pstatus, is_flagged))
            
        if not filtered_posts:
            st.info("No photo posts found in feed.")
        else:
            for pid, ptitle, pcap, pfile, puser, pstatus, is_flagged in filtered_posts:
                # Render Instagram-style Card
                st.markdown(f"""
                <div class="post-card" style="border: 2px solid {'#f87171' if is_flagged else '#dbdbdb'};">
                    <div class="post-header" style="background-color: {'#fef2f2' if is_flagged else '#ffffff'};">
                        <div style="width: 32px; height: 32px; background-color: {'#ef4444' if is_flagged else '#4a90e2'}; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 12px; color: #fff;">
                            {puser[0].upper()}
                        </div>
                        <div class="post-username">
                            {puser} 
                            {f'<span style="color:#ef4444; font-weight:600; margin-left: 10px;">[RESTRICTED AD: {pstatus.upper()}]</span>' if is_flagged else ''}
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    # =================================================================
        ...
        if age_category == "Child":
            ...
            # Inject safe database uploads (only approved, non-restricted)
            for r in db_reels:
                rid, rtitle, rcap, rfile, ruser, rstatus = r
                full_txt = f"{rtitle} {rcap or ''} {rfile}"
                if rstatus == 'approved' and not is_age_restricted(full_txt):
                    reels_feed.append({"title": rtitle, "desc": rcap, "file_path": rfile, "gk": False, "restricted": False, "status": "approved", "username": ruser})
        else:
            st.success("Adult Reels Feed Active. Showing all datasets and ads.")
            # Inject all DB uploads (even restricted/rejected/under_review)
            for r in db_reels:
                rid, rtitle, rcap, rfile, ruser, rstatus = r
                is_flagged = (rstatus in ['rejected', 'under_review']) or is_age_restricted(f"{rtitle} {rcap}")
                reels_feed.append({"title": rtitle, "desc": rcap, "file_path": rfile, "gk": False, "restricted": is_flagged, "status": rstatus, "username": ruser})
            # Inject all datasets
            for ds in VIDEO_SAFETY_DATASETS:
                reels_feed.append(ds)
            # Inject GK reels
        ...
        if not reels_feed:
            ...
            for item in reels_feed:
                ...
                status_label = item.get("status", "REJECTED").upper()
                st.markdown(f"""
                <div class="post-card" style="border: 2px solid {'#f87171' if is_restricted else '#dbdbdb'};">
                    <div class="post-header" style="background-color: {'#fef2f2' if is_restricted else '#ffffff'};">
                        ...
                        <div class="post-username">
                            ...
                            {f'<span style="color:#ef4444; font-weight:600; margin-left: 10px;">[RESTRICTED AD: {status_label}]</span>' if is_restricted else ''}
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
