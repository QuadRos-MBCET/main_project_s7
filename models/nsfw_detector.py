...
class SafeAdNSFWDetector:
    ...
    def _heuristic_skin_eval(self, pil_img: Image.Image) -> float:
        ...
        Filters top half of image to prevent false positives on nature media, skies, and sunsets.
        """
        try:
            ...
            if (upper_skin / total_skin) > 0.60:
                return 0.01

            col_counts = np.count_nonzero(skin_mask, axis=0)
            if (float(np.count_nonzero(col_counts > 0)) / float(width)) > 0.70:
                return 0.01
    ...
    def predict_image(
        ...
        """
        Evaluates a single image / keyframe for adult/NSFW content using pretrained Falconsai ViT.
        """
        ...
        # Check filename/text indicators
        text_indicator = 0.85 if any(k in f"{file_name} {ocr_text}".lower() for k in ["adult", "18+", "nsfw", "sexy", "dating 18+"]) else 0.0

        if self._pipe is not None:
            if normal_score >= 0.50 or (normal_score > nsfw_score and nsfw_score < 0.35):
                final_score = max(nsfw_score, text_indicator)
            else:
